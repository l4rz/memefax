...
async def list_chats(download_all_users=False, download_all_groups=False):
    ...
    try:
        ...
        while True:
            try:
                selected_ids_input = input("\nEnter the ID(s) of the chat(s) to download (comma-separated, or 0 to exit): ")
                if selected_ids_input.strip() == "0":
                    break
                ...
                # Validate all IDs before starting downloads
                invalid_ids = [id for id in selected_ids if id not in chats]
                if invalid_ids:
                    print(f"Invalid chat ID(s): {', '.join(map(str, invalid_ids))}")
                    continue

                # Download messages from all selected chats
                for selected_id in selected_ids:
                    ...
                    if await download_messages(client, selected_chat.entity, chat_dir):
                        print(f"Download completed successfully for {selected_chat.name}")
                    else:
                        print(f"Failed to download messages from {selected_chat.name}")
